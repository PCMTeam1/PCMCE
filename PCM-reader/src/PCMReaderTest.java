...
public class PCMReaderTest {
	...
	/**
	@Test
	public void testPCMForTrip(){
		String fileName = "Comparison_of_free_web_hosting_services.pcm" ;
		PCM p = new PCMReader().getPCM(fileName);
		_LOGGER.info("Mon test" + p.getName());
		EList<Matrix> mesMatrices = p.getMatrices();
		EList<Cell> cells;
		for(Iterator<Matrix> it = mesMatrices.iterator();it.hasNext();){
			Matrix maMatrice = it.next();
			cells = maMatrice.getCells();
					
			for(Cell cell : cells){
				_LOGGER.info("cell.getName() : " +cell.getName());
				
			}

		}
		EList cEList = p.getConcepts();
